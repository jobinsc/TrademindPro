...
import {
  ...
  verifyResetOtp,
  verifyUnlockToken,
} from '@/lib/nexus-pulse-b/strategy-note';
...
function unlockHeader(req: NextRequest): string | null {
  return req.headers.get('x-nexus-b-note-unlock')?.trim() || null;
}
...
export async function POST(req: NextRequest) {
  ...
  if (action === 'save_note') {
    ...
    await saveNoteDoc({
      title: String(body.title || existing.title || 'NexusPulse Sector 7 B Strategy Note'),
