/**
 * NexusPulse Sector 7 B real-option study — Sensex twin of Sector 7 A.
 * Signals: Sensex 3m UT (KV=1, ATR=10) + 5m agree (ATR=14). PnL from Upstox Sensex ATM option 1m closes.
 */
...
export type NexusBRealOptionStudyRun = {
  fromDate: string;
  toDate: string;
...
const STRIKE_STEP = NEXUS_PULSE_RULES.strikeStep;
const SENSEX_UNDERLYING_KEY = SENSEX_INDEX_INSTRUMENT_KEY;
...
class OptionTape {
  ...
  private async allExpiries(): Promise<string[]> {
    ...
    const expired = await fetchExpiredExpiries(this.accessToken, SENSEX_UNDERLYING_KEY);
    await this.sleep(120);
    const live = await fetchOptionContractList(this.accessToken, SENSEX_UNDERLYING_KEY);
    await this.sleep(120);
    const liveEx = [
  ...
  private async contractsFor(expiry: string): Promise<UpstoxOptionContractRow[]> {
    ...
    if (expiry < today) {
      rows = await fetchExpiredOptionContracts(this.accessToken, expiry, SENSEX_UNDERLYING_KEY);
      await this.sleep(120);
    } else {
...
export async function runNexusBRealOptionStudy(opts: {
  accessToken: string;
  fromDate: string;
  toDate: string;
  activeLanes: NexusLaneId[];
  ...
  for (const day of days) {
    ...
    try {
      niftyDay = sessionSlice(
        await fetchInstrumentDayCandles(opts.accessToken, SENSEX_UNDERLYING_KEY, day, todayIso)
      );
    } catch {
      niftyDay = [];
    }
  ...
  const byLane: NexusBRealOptionStudyRun['byLane'] = {};
  for (const laneId of lanes) {
  ...
  const run: NexusBRealOptionStudyRun = {
    ...
    days: summary.days,
    premiumModel: 'REAL Upstox Sensex ATM option 1m close (expired + live FO)',
...
export async function replayNexusBRealOptionsForDay(opts: {
  accessToken: string;
  date: string;
  activeLanes: NexusLaneId[];
}): Promise<{
  ...
  optionFetches: number;
  optionMisses: number;
  byLane: NexusBRealOptionStudyRun['byLane'];
  premiumModel: string;
}> {
