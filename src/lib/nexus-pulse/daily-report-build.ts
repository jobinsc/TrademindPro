...
export const DAILY_REPORT_BROKERAGE = NEXUS_PULSE_RULES.roundTripCostInr;
...
export function grossTrade(t: ReportTradeRow): number {
  ...
  const exitP =
    ...
    t.markPremium ??
    entry;
  const lot = t.lotSize ?? NEXUS_PULSE_RULES.niftyLotSize;
  const qty = t.qty ?? 1;
...
export function buildDailyReportStory(
  ...
  sections: NonNullable<NexusDailyReportMeta['sections']>;
  summary: NexusDailyReportMeta['summary'];
  opening: string[];
  market: string[];
  calc: string[];
  tradeBlocks: string[][];
  ...
  if (!trades.length) {
    market.push('No paper trades on file for this date — run NexusPulse session and close trades, or regenerate after market.');
  ...
  if (!trades.length) {
    deskSummary.push('Desk quiet — start NexusPulse after 09:15 IST with Upstox connected.');
  } else if (totalNet >= 0) {
  ...
  if (!trades.length) {
    suggestions.push('Open NexusPulse, select lane(s), tap Start, and let paper trades archive on close.');
  ...
  const summary: NexusDailyReportMeta['summary'] = {
...
export function metaFromStory(
  ...
  extra?: Partial<NexusDailyReportMeta>
): NexusDailyReportMeta {
  const now = new Date().toISOString();
  return {
    agent: 'NexusPulse',
    date,
    title: `NexusPulse Day Report — ${date}`,
    pdfFile: `NexusPulse-Day-${date}.pdf`,
    pdfPath: `.data/nexus-pulse/reports/daily/NexusPulse-Day-${date}.pdf`,
    generatedAt: now,
