...
const UNLOCK_KEY = 'nexus_pulse_note_unlock_v1';
...
async function api(action: string, body: Record<string, unknown> = {}) {
  ...
  const res = await fetch('/api/nexus-pulse/strategy-note', {
    method: 'POST',
    credentials: 'include',
    headers: {
      'Content-Type': 'application/json',
      ...(unlock ? { 'x-nexus-note-unlock': unlock } : {}),
    },
...
async function fetchStatus() {
  ...
  const res = await fetch('/api/nexus-pulse/strategy-note', {
    credentials: 'include',
    headers: unlock ? { 'x-nexus-note-unlock': unlock } : {},
  });
  const data = (await res.json()) as Record<string, unknown>;
  if (!res.ok || data.ok === false) {
  ...
  async function onSaveNote() {
    ...
    try {
      const data = await api('save_note', {
        title: note?.title || 'NexusPulse Strategy Note',
...
export default function NexusStrategyNoteWorkspace() {
  return (
    <RequireAdmin>
      <StrategyNoteInner />
    </RequireAdmin>
