...
function ArchiveInner() {
  ...
  const loadDates = useCallback(async (m: Mode) => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/nexus-pulse/trade-archive?mode=${m}`, {
        credentials: 'include',
      });
  ...
  const loadDay = useCallback(async (m: Mode, d: string) => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/nexus-pulse/trade-archive?mode=${m}&date=${d}`, {
        credentials: 'include',
      });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[13px] text-sky-ink/60">
            ...
            <code className="text-[11px]">.data/nexus-pulse/trades/</code> — separate from the
...
export default function NexusTradeArchiveWorkspace() {
  return (
    <RequireAdmin>
      <ArchiveInner />
    </RequireAdmin>
