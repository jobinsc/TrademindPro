/**
 * Trader desk context for ATM lab — prior 3 sessions + today's morning read.
 * Decisions must form BEFORE the next leg, like a real intraday trader.
 */
...
function sessionOf(candles: Candle[], date: string): Candle[] {
  return candles
    .filter((c) => istDateKey(c.t) === date)
...
/** Build prior-session map from mixed 1m history (excludes today). */
export function buildAtmTraderContext(
...
/**
 * Morning / intraday desk bias using TODAY only + prior levels.
 * Point-based (not only %), so a clear morning dump is not labelled sideways.
 */
export function readMorningDesk(
  todayCandles: Candle[],
  context: AtmTraderContext | null,
  spot: number
): MorningDeskRead | null {
  ...
  if (swings.lowerHighs && swings.lowerLows) {
    ...
    scoreDown += 1;
    reasons.push(
      swings.lowerHighs ? 'Lower highs printing' : 'Lower lows printing'
    );
  }
  if (swings.higherHighs && swings.higherLows) {
    ...
    scoreUp += 1;
    reasons.push(
      swings.higherHighs ? 'Higher highs printing' : 'Higher lows printing'
    );
  }
  ...
  // Distance from day high after selling = continuation risk for PE
...
export function splitTodayCandles(
  candles: Candle[],
  sessionDate: string
