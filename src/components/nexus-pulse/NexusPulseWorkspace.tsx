...
export default function NexusPulseWorkspace() {
  ...
  const stopPolling = useCallback(() => {
    ...
    if (openN > 0) {
      const ok = window.confirm(
        `Sector 7 A has ${openN} open paper position(s). Stop pauses trail/UT management until you Start again — positions stay saved on disk. Stop anyway?`
      );
      if (!ok) return;
    }
    setNexusDeskArmed('a', false);
  ...
  const clearPaperTrades = useCallback(async () => {
    ...
    if (openN > 0) {
      const ok = window.confirm(
        `Clear will DELETE ${openN} open Sector 7 A paper position(s) and today’s closed book. This cannot be undone. Continue?`
      );
      if (!ok) return;
    }
    setBusy('Clearing…');
  ...
  /** After “Run real-option study”, show study rows in a separate section — never hide live paper. */
