...
import {
  ...
  NEXUS_PULSE_B_NAME as NEXUS_PULSE_NAME,
  NEXUS_PULSE_B_RULES as NEXUS_PULSE_RULES,
  NEXUS_PULSE_B_VERSION as NEXUS_PULSE_VERSION,
  nexusBRuleSummary as nexusRuleSummary,
...
function exitReasonLabel(reason?: NexusPaperTrade['exitReason']) {
  if (!reason) return '—';
  if (reason === 'UT_5M') return 'Sector 7 B';
  if (reason === 'UT_3M') return 'Sector 7 B (3m)';
  return reason;
}
...
export default function NexusPulseBWorkspace() {
  ...
  const saveSettings = useCallback(async (next?: { laneMode?: LaneSelectionMode; stopAfterLossEnabled?: boolean; stopAfterLossInr?: number }) => {
    ...
    try {
      const data = await fetchAppPost<{ ok: boolean; session: NexusPulseSession; error?: string }>({
        path: '/api/nexus-pulse-b/settings',
        token: t,
        body: {
  ...
  const scheduleTick = useCallback(
    (delayMs: number) => {
      ...
      timerRef.current = window.setTimeout(() => {
        ...
        void (async () => {
          try {
            const data = await callApi('/api/nexus-pulse-b/tick');
            if (data.session) {
              setSession((prev) => {
  ...
  const clearPaperTrades = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await callApi('/api/nexus-pulse-b/reset');
  ...
  const runBacktest = useCallback(async (forceRefresh = false) => {
    ...
    try {
      const data = await fetchAppPost<{ ok: boolean; run: NexusBacktestSummary; error?: string }>({
        path: '/api/nexus-pulse-b/backtest',
        token: t,
        body: {
          fromDate: btFrom,
          toDate: btTo,
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      {!live && (
        <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm">
          Connect Upstox in Settings — needed for Sensex 3m/5m + option LTP.
        </p>
      )}
      ...
      <section className="mt-6 flex flex-wrap gap-2">
        {!polling ? (
          ...
          >
            ...
            Start Sector 7 B paper session
          </button>
        ) : (
          <button
            type="button"
      ...
      <section className="mt-4 rounded-2xl border border-sky-100 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[11px] text-sky-ink/55">
              Replays Sector 7 B on Upstox Sensex 1m + real ATM Sensex option 1m closes (last ~31 days max).
              Paper desk matches the Sensex real-option study: strict ATM, closed 3m/5m bars, entries/exits on closed 1m option prints (Lane B default).
            </p>
          </div>
        ...
        <>
          ...
          <section className="mt-4 rounded-2xl border border-sky-100 bg-white p-4 shadow-sm">
            ...
            {session.lastSignal ? (
              ...
              <p className="mt-2 text-[12px] text-sky-ink/55">Waiting for first Sector 7 B tick…</p>
            )}
            <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] md:grid-cols-4">
              <div className="rounded-lg bg-slate-50 p-2">
                <div className="font-semibold">3m Sector 7 B</div>
              ...
              <div className="rounded-lg bg-slate-50 p-2">
                <div className="font-semibold">5m Sector 7 B</div>
          ...
          <section className="mt-6">
            ...
            {visibleOpenTrades.length === 0 ? (
              <p className="mt-2 rounded-xl border border-dashed border-sky-200 bg-sky-50/40 px-3 py-4 text-[13px] text-sky-ink/55">
                Flat on selected lane(s) — no open paper position. Waiting for aligned Sector 7 B entry.
              </p>
            ) : (
