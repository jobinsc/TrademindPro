...
export function JimboSettingsPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  function handleSave(e: React.FormEvent) {
    ...
    updateSettings({
      ...
      enforceMaxLossLimit: Boolean(form.enforceMaxLossLimit),
      enforceDailyTargetLimit: Boolean(form.enforceDailyTargetLimit),
      enforceMaxTradesLimit: Boolean(form.enforceMaxTradesLimit),
      maxTradesPerDay: Math.min(99, Math.max(1, Math.floor(form.maxTradesPerDay || 10))),
